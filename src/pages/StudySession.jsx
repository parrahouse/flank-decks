...
export default function StudySession() {
  ...
  useEffect(() => {
    ...
    const saveStats = async () => {
      ...
      const newMilestoneVal = [3, 7, 14, 30, 60, 100].filter((m) => newStreak >= m).pop() || 0;const prevMilestone = streak?.milestone_reached || 0;if (newMilestoneVal > prevMilestone) {const { toast: sonnerToast } = await import('sonner');
  ...
  if (!filterChosen) {
    ...
    return (
      <div className="relative isolate max-w-5xl mx-auto px-4 py-1">
        ...
        <div className={cn('flex items-center gap-3 mb-6 transition-opacity duration-500', launching && 'opacity-0')}>
          <button
            ...
            className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-card text-foreground shadow-sm transition-colors hover:bg-muted opacity-50">
        ...
        <div className={cn('mx-auto grid w-full max-w-sm gap-6 rounded-[8px] border border-border bg-card p-5 shadow-sm transition-opacity duration-500 lg:max-w-none lg:grid-cols-[minmax(0,384px)_minmax(0,1fr)] lg:gap-8 lg:p-6', launching && 'opacity-0')}>
          ...
          <div className="flex w-full flex-col gap-4">
            <Card className="rounded-[6px] shadow-none">
              <CardHeader className="space-y-0 pr-5 pl-5 pt-3 pb-1">
